...
def generate_amplitude(opt:settings, n_gluons: int, loop_lvl: int):
    ...
    def generate_base_amplitude():
        ...
        for perm in permutation_utils.non_cyclic_perms(range(1,n_gluons + 1)):
            ...
            cs_amp = abstract_cs_amp(opt.cs_amp_letter, loop_lvl, num_idx_list)
    ...
    if loop_lvl == 0:
        return generate_base_amplitude()
    
    if loop_lvl == 1:
        expr = opt.sun_n * generate_base_amplitude()
